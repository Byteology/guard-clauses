...
public static class ObjectExtensions
{
	...
	public static IGuardClause<T> NotNull<T>(this IGuardClause<Nullable<T>> clause) where T : struct
	{
		if (clause.Argument == null)
			throw new ArgumentNullException($"{clause.ArgumentName} should not be null.");

		return new GuardClause<T>(clause.Argument.Value, clause.ArgumentName);
	}
	...
	/// <param name="clause">The guard clause containing the argument to guard.</param>
	/// <exception cref="ArgumentNullException">The argument is <see langword="null"/>.</exception>
	public static IGuardClause<T> NotNull<T>(this IGuardClause<T> clause) where T : class
	{
		if (clause.Argument == null)
			throw new ArgumentNullException($"{clause.ArgumentName} should not be null.");
		...
		string? predicateDescription = null)
	{
		string exceptionMessage =
			predicateDescription == null
				? $"{clause.ArgumentName} should satisfy the specified predicate."
				: $"{clause.ArgumentName} should satisfy the following condition: \"{predicateDescription}\".";
